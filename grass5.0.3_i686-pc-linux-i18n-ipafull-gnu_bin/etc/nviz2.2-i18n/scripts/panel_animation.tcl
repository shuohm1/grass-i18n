...
proc mkanimationPanel { BASE } {
    ...
    set panel [St_create {window name size priority} $BASE [_ "Animation"] 2 5]
    ...
    label $rname.slabel -text [_ "step"]
    ...
    frame $rname -relief sunken
    label $rname.title -text [_ "total frames"]
    ...
    frame $rname -relief sunken
    button $rname.run -text [_ "run"] -width 7 -command "animRunAnimation $BASE"
    button $rname.stop -text [_ "stop"] -width 7 -command {set animRunState stop}
    ...
    label $rname.l -text [_ "Key Frames"]
    ...
    button $rname.add -text [_ "add a keyframe"] -command "animAddKey $BASE"
    button $rname.clear -text [_ "clear all keyframes"] -command "animClearAllKeys $BASE"
    ...
    frame $rname -relief groove
    checkbutton $rname.spath -text [_ "show path"] -variable animPathState \
	-command {Nshow_path $animPathState} -onvalue 1 -offvalue 0
    checkbutton $rname.svect -text [_ "show vect"] -variable animVectState \
	-command {Nshow_vect $animVectState} -onvalue 1 -offvalue 0
    checkbutton $rname.ssite -text [_ "show site"] -variable animSiteState \
    ...
    frame $rname -relief groove
    radiobutton $rname.linear -text [_ "linear"] -variable animInterpType \
	-value linear -command "Nset_interp_mode linear ; Nupdate_frames"
    radiobutton $rname.spline -text [_ "spline -->"] -variable animInterpType \
	-value spline -command "Nset_interp_mode spline ; Nupdate_frames"
    scale $rname.tension -label [_ "tension"] -orient h -showvalue f \
    ...
    button $rname.rands -text [_ "run and save images"] -command "animRunAndSave $BASE" 
...
proc kfKeyPointerMoveBot { BASE tag } {
    ...
    if {$x > $animEndX} then {
	set ans [tk_dialog .verify [_ "Verify"] [_ "Do you really want to delete this keyframe?"] \
		     {} 1 [_ Ok] [_ Dismiss]]
	if {$ans == 1} then {
...
proc animAddKey { BASE } {
	...
	set ans [tk_dialog .replace [_ "Replace Key"] \
		     [_ "There is already a keyframe at this time, do you wish to replace it?"] \
		     {} 1 [_ Ok] [_ Dismiss]]
...
proc animClearAllKeys { BASE } {
    ...
    if {[tk_dialog .verify [_ "Verify"] [_ "Do you really want to delete all keyframes?"] \
	     {} 1 [_ Ok] [_ Dismiss]] == 1} then {
	return
    } else {
	foreach i $animKeyList {
	    set tag1 [lindex $i 5]
...
proc animRunAndSave { BASE } {
    ...
    radiobutton .ras_fname.norm -text [_ "Wireframe"] -variable animSaveRenderStyle -value 0
    radiobutton .ras_fname.fancy -text [_ "Full Rendering"] -variable animSaveRenderStyle -value 1
    button .ras_fname.ok -text [_ "Ok"] -command "set animWaitPress true"
