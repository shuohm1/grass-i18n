...
proc mklightsPanel { BASE } {
    ...
    if [catch {set Nv_($BASE)}] {
        set panel [St_create {window name size priority} $BASE [_ "Lights"] 2 5]
    } else {
	set panel $Nv_($BASE)
    }
    frame $BASE  -relief groove -borderwidth 2
    Nv_mkPanelname $BASE [_ "Lighting Panel"]
    ...
    checkbutton $BASE.top.left.follow -relief flat -text [_ "Follow ViewPoint"] \
	-variable Nv_(FollowView) -command follow -onvalue 1 -offvalue 0
    checkbutton $BASE.top.left.show -relief flat -text [_ "Show Model"] \
	-variable Nv_(ShowModel)
    Nv_mkScale $BASE.top.left.bright h [_ Brightness] 100 0 80 set_brt 2 
    Nv_mkScale $BASE.top.left.ambient h [_ Ambient] 100 0 20 set_amb 2
    pack $BASE.top.left.follow $BASE.top.left.show \
        $BASE.top.left.bright $BASE.top.left.ambient \
	-side top -fill x -expand 1
    
    Nv_mkScale $BASE.top.right.red h [_ Red] 100 0 100 set_red 2
    Nv_mkScale $BASE.top.right.green h [_ Green] 100 0 100 set_green 2
    Nv_mkScale $BASE.top.right.blue h [_ Blue] 100 0 100 set_blue 2
    ...
    pack [Nv_mkScale $BASE.bottom.height v [_ Height] 100 0 80 set_lgt_hgt 2] \
