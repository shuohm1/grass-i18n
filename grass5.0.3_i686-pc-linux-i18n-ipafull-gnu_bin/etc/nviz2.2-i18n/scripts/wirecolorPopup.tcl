...
proc mkWireColorPopup {w name {color "#000000"} {mode 0}} {
    ...
    wm title $w [_ "Select Color"]
    wm iconname $w [_ "Color"]
    ...
    checkbutton $w.usemap -text [_ "Use Surface Color"] -onvalue UseMap \
	-offvalue 0 -variable CurrWireColor
    pack $w.top $w.bot $w.usemap -side top -fill both -expand yes

    frame $w.top.left
    Nv_mkWireColorScale $w.top.left [_ Red] $r red  $w.top.color
    Nv_mkWireColorScale $w.top.left [_ Green] $g green  $w.top.color
    Nv_mkWireColorScale $w.top.left [_ Blue] $b blue  $w.top.color
    ...
    button $w.bot.buttonframe.ok -text [_ OK] -command "destroy $w"
    button $w.bot.buttonframe.cancel  \
	-text [_ Cancel]  -command "set CurrWireColor $tmp; destroy $w"
