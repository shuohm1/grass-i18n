...
proc mksitePanel { BASE } {
	...
	set panel [St_create {window name size priority} $BASE [_ "Sites"] 1 5]
    } else {
	set panel $Nv_($BASE)
    }

    frame $BASE  -relief groove -borderwidth 2
    Nv_mkPanelname $BASE [_ "Site Panel"]

    set tmp [frame $BASE.top]
    label $tmp.current -text [_ "Current:"] -anchor nw
    mkMapList $tmp.list site 
    button $tmp.new  -text [_ New] -anchor ne -command "add_map site"
    button $tmp.delete  -text [_ Delete] -anchor ne -command "delete_map site"
    ...
    frame $BASE.f 
    button $BASE.f.close -text [_ Close] \
	-command "Nv_closePanel $BASE" -anchor s
    button $BASE.f.color -text [_ Color] -anchor s \
	-command "change_color site $BASE.f.color"
    button $BASE.f.draw_current -text [_ "Draw Current"] -anchor s \
    ...
    Nv_mkFloatScale $tmp.sitesize h [_ "site size"] $longdim \
	0 $size change_site_size 2
    Nv_mkArrows $tmp.linewidth [_ "Line Width"] [concat set_width site] $width 
    
    frame $tmp.siteshape
    
    radiobutton $tmp.siteshape.x -relief flat -text [_ "use X"] -value x \
	-anchor nw -variable Nv_(siteshape) -command change_marker
    radiobutton $tmp.siteshape.sphere -relief flat -text [_ "use sphere"] \
	-value sphere -anchor nw -variable Nv_(siteshape) \
	-command change_marker
    radiobutton $tmp.siteshape.diamond -relief flat -text [_ "use diamond"] \
    ...
    frame $tmp.sitedisp
    radiobutton  $tmp.sitedisp.threed -text [_ "3D Sites"] \
	-anchor nw -variable Nv_(sitedisplay) -value 3d \
	-command change_site_mode
    radiobutton  $tmp.sitedisp.surfdisp -text [_ "Display on surface(s):"] \
