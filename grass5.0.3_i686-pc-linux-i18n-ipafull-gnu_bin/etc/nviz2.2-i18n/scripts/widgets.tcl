...
proc Nv_mkXYScale {C {type puck} {name null} {height 100} {width 100} {x 50} {y 50} {cmd null} {upcmd null}} {
    ...
    #Draw North Arrow
        $C create text [expr $width - 2] [expr $height/2] -text [_ E] -fill black
	$C create text 3 [expr $height/2] -text [_ W] -fill black -anchor w
	$C create text [expr $width/2] 2 -text [_ N] -fill black -anchor n
	$C create text [expr $width/2] $height -text [_ S] -fill black -anchor s
...
proc Nv_mkMenu { P mname bnames underlines commands} {
    ...
    menubutton $P -text [_ $mname] -menu $P.m -underline [expr [string length [_ "$mname"]] - 2]
    menu $P.m
    set j 0
    foreach i $bnames { 
	...
	$P.m add cascade -label [_ $i] -underline [expr [string length [_ "$i"]] - $underline] -menu \
	$P.m.$menu_name 
	$menu_build $P.m.$menu_name
	} else {
	$P.m add command -label [_ $i] -underline [expr [string length [_ "$i"]] - $underline] -command \
	    "inform [concat $i]; $cmd" 
	}
	incr j
    }
...
proc Nv_mkPanelname {P name} {
    ...
    label $P.name.label -text [_ $name]
...
proc Nv_mkAttbutton {P name} {
    frame $P.$name
    button $P.$name.b -text [_ $name] -anchor nw -width 10 \
	-command "mkAttPopup $P.$name.pop $name 1"
    
    set txt [get_curr_status $name]
    
    label $P.$name.l -text [_ $txt] -anchor ne
...
proc Nv_mkSurfacelist { P L C type} {
	...
	checkbutton $P.$j  -relief flat -text [_ $name] -anchor w\
...
proc mkMapList { P type {cmd null}} {
    ...
    menubutton $P -text [_ $name] -menu $P.m
    menu $P.m -tearoff 0
    foreach i $list {
