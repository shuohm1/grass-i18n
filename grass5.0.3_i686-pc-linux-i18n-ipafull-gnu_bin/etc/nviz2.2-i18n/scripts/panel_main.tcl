...
proc mkmainPanel { BASE } {
    ...
    set labl1 [label $BASE.redrawf.f1.label1  -text [_ Auto:] ]
    set auto [checkbutton $BASE.redrawf.f1.autoclear -text [_ "Clear"] \
		  -variable autoc ] 
    $auto select
    set auto_d [checkbutton $BASE.redrawf.f1.autodraw -text [_ "Draw"] \
                  -onvalue 1 -offvalue 0 -variable auto_draw ]
    $auto_d select
    pack $labl1 $auto $auto_d -side left -expand 1 -fill x

#checkbuttons for features to draw
    set labl2 [label $BASE.redrawf.f11.label1  -text [_ Feature:] ]

    set surf_b [checkbutton $BASE.redrawf.f11.surface -text [_ "Surface"] \
                  -onvalue 1 -offvalue 0 -variable surface]
    $surf_b select

    set vect_b [checkbutton $BASE.redrawf.f11.vector -text [_ "Vectors"] \
                  -onvalue 1 -offvalue 0 -variable vector]
    $vect_b select

    set site_b [checkbutton $BASE.redrawf.f11.sites -text [_ "Sites"] \
...
#Execute buttons
    ...
    button $BASE.redrawf.f2.exec -text [_ DRAW] -command { \
   ...
   button $BASE.redrawf.f2.clear -text [_ Clear] -command {do_clear}

   button $BASE.redrawf.f2.cancel -text [_ Cancel] -command {Nset_cancel 1}
...
#pack frames
    ...
    label $BASE.midf.lookat.l -text [_ LOOK]

#button $BASE.midf.lookat.here -text [_ here] \
#-command {bind .top.canvas <Button> {if [%W islinked] {look_here %W %x %y}}}
    ...
    button $BASE.midf.lookat.center -text [_ center] -command { look_center
	if {[Nauto_draw] == 1} {Ndraw_all} }
    button $BASE.midf.lookat.top -text [_ top] -command { Nv_itemDrag $Nv_(main_BASE).midf.pos $Nv_(XY_POS) 62.5 62.5
# note: below value is somewhat strange, but with 0.5 0.5 the map rotates:
	update_position 0.496802 0.50100
	update
        if {[Nauto_draw] == 1} {Ndraw_all} }
    button $BASE.midf.lookat.cancel -text [_ cancel] -command no_focus
    ...
    frame $BASE.bframe
    set P [Nv_mkScale $BASE.bframe.pers h [_ perspective] 120 3 40 Nchange_persp 0]
    button $BASE.bframe.reset -text [_ RESET] -command "do_reset $XY $H $E $P"
...
#	}
    ...
    Nv_mkFloatScale $W.zexag v [_ zexag] $exag $min $val update_exag 2
...
proc mk_hgt_slider {W} {
    ...
    Nv_mkFloatScale $W.height v [_ height] $max $min $val update_height 2
