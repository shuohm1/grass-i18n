...
proc mkAttPopup {w att {mode 0}} {
    ...
    label $w.f1.name -text "[_ "Change Attribute:"] [_ $att]"
    pack $w.f1.name -side top -fill both -expand yes 
    button $w.f2.map -text [_ "New Map"] -command "$cb1"
    
    if {"$att" == "mask"} then {
	button $w.f2.const -text [_ "Remove Mask"] -command "$cb2"
	checkbutton $w.f2.invert -text [_ "Invert Mask"] -onvalue 1 \
	...
	button $w.f2.const -text [_ "New Constant"]	-command "$cb2"
	checkbutton $w.f2.use_color -text [_ "Use as color"] -onvalue 1 \
    ...
    label $w.f3.status -text [_ "Status: "]
    ...
    button $w.f4.accept -text [_ "Accept"] \
	-command "ap_check_invert $att ; destroy $w"
    button $w.f4.cancel -text [_ "Cancel"] \
...
proc get_curr_status {att} {
	...
	if {![regexp {[a-z,A-Z,"("]} $txt]} then {
	    ...
	    set txt "[_ "color:"] [_ "R"]$red, [_ "G"]$green, [_ "B"]$blue"
	}
    }
    return $txt
}
...
proc ap_get_colorconst {} {
    ...
    set new_color [mkColorPopup .color_browse [_ "Surface Color"] $curr_color 1]
    ...
    set attPopup_Status "[_ "color:"] [_ "R"]$red, [_ "G"]$green, [_ "b"]$blue"
...
proc ap_remove_mask {} {
    ...
    set attPopup_Status [_ "mask not set"]
    set attPopup_Type constant
}
...
proc ap_check_invert {att} {
    ...
    if {"$attPopup_Status" == [_ "mask not set"]} then {
	return
    }
...
proc create_constant_popup {{w .enter_constant} {mode 0}} {
    ...
    toplevel $w
    wm title $w [_ "new_const"]
    tkwait visibility $w

    puts "CONSTANT: $w MODE: $mode"
    label $w.title -bd 2 -text [_ "Enter a constant value :"] -relief raised
...
proc create_slideconstant_popup {{w .enter_constant} {mode 0}} {
    ...
    puts "CONSTANT: $w MODE: $mode"
    label $w.title -bd 2 -text [_ "Use slider to select constant value :"] -relief raised
    scale $w.constant -from 0 -to 255 -showvalue yes \
	-orient horizontal -activebackground gray80 \
	-background gray90
    button $w.close -text [_ "Done"] -command "set cp_done 1"
