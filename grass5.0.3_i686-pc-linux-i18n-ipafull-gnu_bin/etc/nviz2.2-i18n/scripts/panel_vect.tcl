...
proc mkvectPanel { BASE } {
	...
	set panel [St_create {window name size priority} $BASE [_ "Vectors"] 1 5]
    } else {
	set panel $Nv_($BASE)
    }
    frame $BASE -relief groove -borderwidth 2
    Nv_mkPanelname $BASE [_ "Vector Panel"]
    
    set tmp [frame $BASE.top]
    label $tmp.current -text [_ "Current:"] -anchor nw
    mkMapList $tmp.list vect 
    button $tmp.new -text [_ New] -anchor ne -command "add_map vect" 
    button $tmp.delete -text [_ Delete] -anchor ne -command "delete_map vect"
    ...
    frame $BASE.f 
    button $BASE.f.close -text [_ Close] -command "Nv_closePanel $BASE" -anchor s
    ...
    Nv_mkArrows $tmp.linewidth [_ "Line Width"] [concat set_width vect] $width
#	checkbutton $tmp.load -relief flat -text [_ "Load to memory"]
    button $tmp.color -text [_ Color] \
    ...
    label $tmp.label -text [_ "Display on surface(s):"]
