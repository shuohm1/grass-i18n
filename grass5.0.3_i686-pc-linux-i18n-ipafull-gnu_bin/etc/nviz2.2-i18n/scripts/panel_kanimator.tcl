...
global keyanimVectState keyanimSiteState
...
proc mkkanimatorPanel { BASE } {
    ...
    set panel [St_create {window name size priority} $BASE [_ "Keyframe Animation"] 2 5]
    Nv_mkPanelname $BASE [_ "Animation Panel"]
    ...
    button $rname.back         -bitmap @$bit_map_path/left \
	...
	$rname.forward -side left -padx 2 -pady 2 -fill both -expand yes
    pack $rname -side left -fill both -expand yes

    frame $BASE.framerate -relief groove -borderwidth 2
    set rname $BASE.framerate
    button $rname.label -text [_ "Framerate :"] -command "keyanimChangeFramerate $BASE"
    label  $rname.val   -textvariable keyanimFrameRate
    pack $rname.label $rname.val -side left -padx 2 -pady 2 -fill both -expand yes
    pack $rname -side left -fill both -expand yes
    
    frame $BASE.commands -relief groove -borderwidth 2
    set rname $BASE.commands
    button $rname.add 		-text [_ "Add"] 	-command "keyanimAddKey $BASE"
    button $rname.delete 	-text [_ "Delete"] 	-command "keyanimDeleteKeys $BASE"
    button $rname.close 	-text [_ "Close"] 	-command "Nv_closePanel $BASE"
    button $rname.save          -text [_ "Save"]    -command "keyanimSaveAnim $BASE"
    button $rname.load          -text [_ "Load"]    -command "keyanimLoadAnim $BASE"
    button $rname.rands		-text [_ "Run and Save Images"] \
	-command "keyanimRunAndSave $BASE"
	
    pack $rname.rands -side bottom -fill both -expand yes -padx 2 -pady 2
    pack $rname.add $rname.delete $rname.close $rname.save $rname.load \
	-side left -padx 2 -pady 2 -fill both -expand yes
    pack $rname -side bottom -before $BASE.playcontrol -fill both -expand yes
    ...
    mkkeyframeArea $BASE
    pack $BASE.keycontrol -padx 3 -pady 3 -fill both -side bottom \
	-before $BASE.playcontrol
    
    # Create bottom section containing current keytime
    frame $BASE.keytime -relief groove -borderwidth 2
    set rname $BASE.keytime
    button $rname.change -text [_ "New Key Time:"] -command "keyanimChangeKeytime $BASE"
    label $rname.current -textvariable keyanimCurrentKeyTime
    pack $rname.change $rname.current -side left -padx 2 -pady 2 -fill both -expand yes
    pack $rname -side bottom -before $BASE.keycontrol -fill both -expand yes
    ...
    checkbutton $rname.spath -text [_ "show path"] -variable keyanimPathState \
	-command {Nshow_path $keyanimPathState} -onvalue on -offvalue off
    checkbutton $rname.svect -text [_ "show vect"] -variable keyanimVectState \
	-command {Nshow_vect $keyanimVectState} -onvalue on -offvalue off
    checkbutton $rname.ssite -text [_ "show site"] -variable keyanimSiteState \
	-command {Nshow_site $keyanimSiteState} -onvalue on -offvalue off
    pack $rname.spath $rname.svect $rname.ssite -fill both \
	-padx 2 -pady 2 -expand yes -side left
    
    pack $BASE.other_commands -after $BASE.commands \
	-fill both -expand yes -side bottom
    
    set rname $BASE.other_commands.interp
    frame $rname -relief groove
    radiobutton $rname.linear -text [_ "linear"] \
	-variable keyanimInterpType -value linear \
	-command "Nset_interp_mode linear ; Nupdate_frames"
    radiobutton $rname.spline -text [_ "spline -->"] \
	-variable keyanimInterpType -value spline \
	-command "Nset_interp_mode spline ; Nupdate_frames"
    
    scale $rname.tension -label [_ "tension"] -orient h \
	-showvalue f -from 0 -to 1000 -command keyanimChangeTension \
	-activebackground gray80 -background gray90

    $rname.tension set 500

    pack $rname.linear $rname.spline $rname.tension \
	-side left -anchor s -fill both -padx 2 -pady 2 -expand yes
    pack $rname -side bottom -padx 2 -pady 2 \
	-before $BASE.other_commands.spath -expand yes
    
    # Add all animation channels
    keyanimAddChannel $BASE [_ "FromX"]
    keyanimAddChannel $BASE [_ "FromY"]
    keyanimAddChannel $BASE [_ "FromZ"]
    keyanimAddChannel $BASE [_ "DirX"] 
    keyanimAddChannel $BASE [_ "DirY"] 
    keyanimAddChannel $BASE [_ "DirZ"] 
    keyanimAddChannel $BASE [_ "FOV"]  
    keyanimAddChannel $BASE [_ "TWIST"]
...
proc keyanimAddChannel { BASE element {entry_list null} {get_entries null} {set_entries null} } {
    ...
    checkbutton $rname.active -text [_ "On"] -variable $channel_iname
...
proc keyanimChangeKeytime { BASE } {
    ...
    toplevel $name
    label $name.label -text [_ "New Key Time"]
    pack $name.label
    
    frame $name.lgroup -relief groove
    label $name.lgroup.mlabel -text [_ "Minute:"] -relief raised
    label $name.lgroup.slabel -text [_ "Second:"] -relief raised
    label $name.lgroup.flabel -text [_ "Frame:"] -relief raised
    ...
    button $name.ok -text [_ "Ok"] -command "set button 1"
	...
	tk_dialog .kt_error [_ "KeyTime Error"] [_ "Error - All values must be at least zero."] \
	    {} 0 [_ Dismiss]
	destroy $name
	focus $oldFocus
	return
	...
	tk_dialog .kt_error [_ "KeyTime Error"] \
	    [_ "Error - Frame number must be less than frame rate."] \
	    {} 0 [_ Dismiss]
	destroy $name
	focus $oldFocus
	return
...
proc keyanimChangeFramerate { BASE } {
    ...
    toplevel $name
    label $name.label -text [_ "New Frame Rate"]
    pack $name.label
    
    frame $name.lgroup -relief groove
    label $name.lgroup.flabel -text [_ "Framerate:"] -relief raised
    ...
    if {$framerate < 1} then {
	tk_dialog .fr_error [_ "Frame Rate Error"] [_ "Error - Frame rate must be at least 1."] \
	    {} 0 [_ Dismiss]
	destroy $name
	focus $oldFocus
	return
...
proc keyanimDrawKeys { BASE {old_tags {} } } {
	...
	foreach j [lindex $i 1] {
	    ...
	    if {$tag_name == ""} then {
		tk_dialog .ierror [_ "Internal Error"] \
		    "[_ "Internal Error - Can't find channel"] $cname [_ "for keyframe"] $i" \
		    {} 0 [_ Dismiss]
		return
	    }
...
proc keyanimDeleteKeys { BASE } {
    ...
    set ans [tk_dialog .delete_keys [_ "Delete Keys"] \
		 [_ "Delete the selected keys?"] \
		 {} 1 [_ Ok] [_ Dismiss]]
	...
	if {[Ndelete_key $internal_time 0 1] == 0} then {
	    tk_dialog .ierror [_ "Internal Error"] \
		[_ "Internal Error - Failed to delete keyframe in GK key list"] \
		{} 0 [_ Dismiss]
	}
	Nupdate_frames
...
proc keyanimMoveKeyRelease { BASE x tag } {
	...
	tk_dialog .ierror [_ "Internal Error"] \
	    [_ "Internal Error - Failed to move keyframe in GK key list"] \
	    {} 0 [_ Dismiss]
    }
...
proc keyanimAddKey { BASE } {
	...
	set ans [tk_dialog .replace [_ "Replace Key"] \
		     [_ "There is already a keyframe at this time, replace it?"] \
		     {} 1 [_ Ok] [_ Dismiss]]
	if {$ans == 1} then { return }
...
proc mkkeyframeArea { BASE } {
	...
	-text [_ "On"] -state disabled -relief flat -bd 0 -padx 3
...
proc keyanimRunAndSave { BASE } {
    ...
    set IMG 2
    toplevel .ras_fname
    wm title .ras_fname [_ "ras_fname"]
    frame .ras_fname.frame1
    frame .ras_fname.frame2
    label .ras_fname.title -text [_ "Enter a base name:"]
    entry .ras_fname.enter -relief sunken
    radiobutton .ras_fname.norm -text [_ "Wireframe"] \
	-variable keyanimSaveRenderStyle -value 0
    radiobutton .ras_fname.fancy -text [_ "Full Rendering"] \
	-variable keyanimSaveRenderStyle -value 1
    button .ras_fname.ok -text [_ "Ok"] -command "set keyanimWaitPress true"
    label .ras_fname.label -text "" -relief raised
    radiobutton .ras_fname.img1 -text [_ "Iris RGB"] -variable IMG -value 1
    radiobutton .ras_fname.img2 -text [_ "PPM"] -variable IMG -value 2
    radiobutton .ras_fname.img3 -text [_ "TIFF"] -variable IMG -value 3
...
proc keyanimSaveAnim { base } {
    ...
    if {[llength $keyanimKeyList] == 0} then {
	tk_dialog .na_error [_ "No Anim Error"] \
	    [_ "There are no keys in the current animation to save"] \
	    {} 0 [_ Dismiss]
	return
    }
...
proc keyanimLoadAnim { base } {
	...
	set status [tk_dialog .ka_check [_ "Verify"] \
			[_ "Warning - Current animation has not been saved, continue?"] \
			{} 1 [_ Ok] [_ Cancel]]
	if {$status == 1} return
    }
	...
	tk_dialog .ka_error [_ "File Error"] \
	    "[_ "Error - Could not open file"] $file_name [_ "for reading"]" \
	    {} 0 [_ Dismiss]
	return 
    }
